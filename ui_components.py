...
# ════════════════════════════════════════════════════════════════════
# BROKERAGE-STYLE COMBINED OPTIONS CHAIN
# ════════════════════════════════════════════════════════════════════

def render_combined_options_chain(calls_df, puts_df, current_price, strike_count=16):
    """Render a standard brokerage options chain: CALLS | STRIKE | PUTS
    
    Layout rules (matches thinkorswim / IB / Robinhood):
    - Single shared strike column centered around ATM
    - Lower strikes at top, higher strikes at bottom
    - Calls ITM (strike < price): highlighted green on left
    - Puts  ITM (strike > price): highlighted red on right
    - ATM row: orange highlight, centered with markers
    - Strike count evenly split above/below ATM (or 1 ATM center if odd)
    """
    if calls_df is None or puts_df is None:
        return '<p style="color:#555;font-family:monospace;font-size:11px">No data available</p>'
    import pandas as pd
    ...
    # Build full sorted strike list from both sides
    all_strikes = sorted(set(
        calls["strike"].dropna().tolist() + puts["strike"].dropna().tolist()
    ))
    if not all_strikes or current_price <= 0:
        return '<p style="color:#555;font-family:monospace;font-size:11px">Insufficient data</p>'

    # ── Find ATM: strike closest to current_price
    atm_strike = min(all_strikes, key=lambda s: abs(s - current_price))
    atm_idx    = all_strikes.index(atm_strike)

    # ── Select strike_count strikes mathematically centered around ATM
    # For even count: half above, half below (ATM itself is the midpoint boundary)
    # For odd  count: half above, ATM center, half below
    half = strike_count // 2
    if strike_count % 2 == 0:
        # Even: half strikes BELOW atm_idx, half AT/ABOVE atm_idx
        lo_idx = atm_idx - half
        hi_idx = atm_idx + half - 1
    else:
        # Odd: ATM is center
        lo_idx = atm_idx - half
        hi_idx = atm_idx + half
    ...
    lo_idx = max(0, lo_idx)
    hi_idx = min(len(all_strikes) - 1, hi_idx)

    display_strikes = all_strikes[lo_idx : hi_idx + 1]

    # ── Build fast lookup by strike (find closest match, not exact float equal)
    def _best_row(df, strike, tol=0.005):
        """Return row dict with smallest strike distance within tolerance, or None."""
        if df.empty: return None
        df2 = df.copy()
        df2["_d"] = (df2["strike"] - strike).abs()
        best = df2.loc[df2["_d"].idxmin()]
        if best["_d"] <= max(tol, strike * 0.001):
            return best.to_dict()
        return None

    def _cell(row, field):
        """Format a cell value for the options table."""
        if row is None: return '<span style="color:#333">—</span>'
        v = row.get(field, None)
        if v is None or (isinstance(v, float) and pd.isna(v)):
            return '<span style="color:#333">—</span>'
        try:
            fv = float(v)
            if pd.isna(fv): return '<span style="color:#333">—</span>'
            if field == "impliedVolatility":
                return f"{fv:.1%}"
            if field in ("volume", "openInterest"):
                iv = int(fv)
                if iv >= 1_000_000: return f"{iv/1e6:.1f}M"
                if iv >= 1_000: return f"{iv/1_000:.0f}K"
                return f"{iv:,}"
            return f"{fv:.2f}"
        except Exception:
            return '<span style="color:#333">—</span>'
    ...
    for strike in display_strikes:
        call = _best_row(calls, strike)
        put  = _best_row(puts,  strike)

        is_atm   = abs(strike - atm_strike) < 0.005
        call_itm = current_price > strike   # call ITM when stock price > strike
        put_itm  = current_price < strike   # put  ITM when stock price < strike

        # Row background & strike styling
        if is_atm:
            row_bg      = "background:rgba(255,102,0,0.12)"
            strike_style = "color:#FF6600;font-weight:900;font-size:12px;text-align:center"
            strike_label = f"▶ {strike:.2f} ◀"
        else:
            row_bg       = "background:rgba(0,204,68,0.03)" if call_itm else ""
            strike_style = "color:#888;font-weight:600;text-align:center"  if call_itm else "color:#444;text-align:center"
            strike_label = f"{strike:.2f}"

        # Side colors
        cc = "#00CC44" if call_itm else "#888"
        pc = "#FF4444" if put_itm  else "#888"
        c_bg = "background:rgba(0,204,68,0.05)" if call_itm else ""
        p_bg = "background:rgba(255,68,68,0.05)" if put_itm else ""

        rows_html += (
            f'<tr style="{row_bg}">'
            # ── CALLS side (right-to-left: IV OI VOL BID ASK LAST → towards strike)
            f'<td style="{c_bg};color:{cc};text-align:right;padding:3px 5px">{_cell(call,"impliedVolatility")}</td>'
            f'<td style="{c_bg};color:{cc};text-align:right;padding:3px 5px">{_cell(call,"openInterest")}</td>'
            f'<td style="{c_bg};color:{cc};text-align:right;padding:3px 5px">{_cell(call,"volume")}</td>'
            f'<td style="{c_bg};color:{cc};text-align:right;padding:3px 5px">{_cell(call,"bid")}</td>'
            f'<td style="{c_bg};color:{cc};font-weight:600;text-align:right;padding:3px 5px">{_cell(call,"ask")}</td>'
            f'<td style="{c_bg};color:{cc};font-weight:700;text-align:right;padding:3px 8px">{_cell(call,"lastPrice")}</td>'
            # ── STRIKE center
            f'<td style="{strike_style};padding:3px 10px;border-left:1px solid #1A1A1A;border-right:1px solid #1A1A1A">{strike_label}</td>'
            # ── PUTS side (left-to-right: LAST BID ASK VOL OI IV → away from strike)
            f'<td style="{p_bg};color:{pc};font-weight:700;text-align:left;padding:3px 8px">{_cell(put,"lastPrice")}</td>'
            f'<td style="{p_bg};color:{pc};font-weight:600;text-align:left;padding:3px 5px">{_cell(put,"bid")}</td>'
            f'<td style="{p_bg};color:{pc};text-align:left;padding:3px 5px">{_cell(put,"ask")}</td>'
            f'<td style="{p_bg};color:{pc};text-align:left;padding:3px 5px">{_cell(put,"volume")}</td>'
            f'<td style="{p_bg};color:{pc};text-align:left;padding:3px 5px">{_cell(put,"openInterest")}</td>'
            f'<td style="{p_bg};color:{pc};text-align:left;padding:3px 5px">{_cell(put,"impliedVolatility")}</td>'
            f'</tr>'
        )

    # ── Current price separator row
    price_row = (
        f'<tr style="background:rgba(255,102,0,0.25);border-top:2px solid #FF6600;border-bottom:2px solid #FF6600">'
        f'<td colspan="6" style="text-align:right;padding:2px 8px;font-family:monospace;font-size:10px;color:#FF6600;font-weight:700">'
        f'◄ CALLS ITM &nbsp; | &nbsp; CURRENT: ${current_price:.2f}</td>'
        f'<td style="text-align:center;padding:2px 4px;color:#FF6600;font-weight:900;font-size:11px">—</td>'
        f'<td colspan="6" style="text-align:left;padding:2px 8px;font-family:monospace;font-size:10px;color:#FF6600;font-weight:700">'
        f'${current_price:.2f} &nbsp; | &nbsp; PUTS ITM ►</td>'
        f'</tr>'
    )

    # Insert price separator at the ATM position
    # We rebuild rows_html with the separator inserted after the last ITM call row
    rows_list = rows_html.split('</tr>\n')
    # Find insertion point: after last call-ITM row (call_itm = strike < current_price)
    insert_after = 0
    for i, s in enumerate(display_strikes):
        if s < current_price:
            insert_after = i
    rows_lines = []
    for i, strike in enumerate(display_strikes):
        call = _best_row(calls, strike)
        put  = _best_row(puts, strike)
        is_atm   = abs(strike - atm_strike) < 0.005
        ...
        # Insert price separator after the last strike that is below current price
        if i == insert_after and not is_atm:
            rows_lines.append(price_row)

    final_rows = "\n".join(rows_lines)

    return (
        f'<div style="overflow-x:auto;font-family:IBM Plex Mono,monospace">'
        f'<table style="border-collapse:collapse;width:100%;font-size:11px">'
        f'<thead>'
        f'<tr>'
        f'<th colspan="6" style="text-align:center;padding:5px 8px;color:#00CC44;background:rgba(0,204,68,0.06);'
        f'border-bottom:2px solid #00CC44;font-size:10px;letter-spacing:2px">▲ CALLS (ITM ← ATM → OTM)</th>'
        f'<th style="text-align:center;padding:5px 10px;color:#FF6600;background:rgba(255,102,0,0.08);'
        f'border-bottom:2px solid #FF6600;font-size:10px;letter-spacing:2px">STRIKE</th>'
        f'<th colspan="6" style="text-align:center;padding:5px 8px;color:#FF4444;background:rgba(255,68,68,0.06);'
        f'border-bottom:2px solid #FF4444;font-size:10px;letter-spacing:2px">▼ PUTS (OTM ← ATM → ITM)</th>'
        f'</tr>'
        f'<tr style="background:#080808">'
        f'<th style="text-align:right;padding:3px 5px;color:#555;font-size:9px;font-weight:400">IV</th>'
        f'<th style="text-align:right;padding:3px 5px;color:#555;font-size:9px;font-weight:400">OI</th>'
        f'<th style="text-align:right;padding:3px 5px;color:#555;font-size:9px;font-weight:400">VOL</th>'
        f'<th style="text-align:right;padding:3px 5px;color:#555;font-size:9px;font-weight:400">BID</th>'
        f'<th style="text-align:right;padding:3px 5px;color:#555;font-size:9px;font-weight:400">ASK</th>'
        f'<th style="text-align:right;padding:3px 8px;color:#555;font-size:9px;font-weight:400">LAST</th>'
        f'<th style="text-align:center;padding:3px 10px;color:#555;font-size:9px;border-left:1px solid #1A1A1A;border-right:1px solid #1A1A1A;font-weight:400">—</th>'
        f'<th style="text-align:left;padding:3px 8px;color:#555;font-size:9px;font-weight:400">LAST</th>'
        f'<th style="text-align:left;padding:3px 5px;color:#555;font-size:9px;font-weight:400">BID</th>'
        f'<th style="text-align:left;padding:3px 5px;color:#555;font-size:9px;font-weight:400">ASK</th>'
        f'<th style="text-align:left;padding:3px 5px;color:#555;font-size:9px;font-weight:400">VOL</th>'
        f'<th style="text-align:left;padding:3px 5px;color:#555;font-size:9px;font-weight:400">OI</th>'
        f'<th style="text-align:left;padding:3px 5px;color:#555;font-size:9px;font-weight:400">IV</th>'
        f'</tr>'
        f'</thead>'
        f'<tbody>{final_rows}</tbody>'
        f'</table></div>'
    )
