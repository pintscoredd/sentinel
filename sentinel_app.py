...
with tabs[0]:
    ...
    with L:
        ...
        # ── Header — use same st.columns proportions as data rows ──
        _hdr = st.columns([1.5, 2.0, 1.7, 1.7, 1.5, 0.8])
        _hdr_labels = ["TICKER", "PRICE", "CHG %", "CHG $", "VOLUME", "DEL"]
        for _col, _lbl in zip(_hdr, _hdr_labels):
            _col.markdown(
                f'<div style="font-family:monospace;font-size:9px;color:#FF6600;'
                f'letter-spacing:1px;padding:4px 0;border-bottom:1px solid #FF6600">{_lbl}</div>',
                unsafe_allow_html=True
            )
        ...
        for q in wl_qs:
            ...
            with crow[0]: st.markdown(f'<div style="color:#FF6600;font-weight:700;font-family:monospace;font-size:13px;padding:6px 0">{q["ticker"]}</div>', unsafe_allow_html=True)
            with crow[1]: st.markdown(f'<div style="color:#FFF;font-family:monospace;font-size:13px;padding:6px 0">{fmt_p(q["price"])}</div>', unsafe_allow_html=True)
            with crow[2]: st.markdown(f'<div style="color:{c};font-family:monospace;font-size:13px;padding:6px 0">{arr} {abs(q["pct"]):.2f}%</div>', unsafe_allow_html=True)
            with crow[3]: st.markdown(f'<div style="color:{c};font-family:monospace;font-size:13px;padding:6px 0">{chg_str}</div>', unsafe_allow_html=True)
            with crow[4]: st.markdown(f'<div style="color:{c};font-family:monospace;font-size:13px;padding:6px 0">{vol}</div>', unsafe_allow_html=True)
            with crow[5]:
