...
# ════════════════════════════════════════════════════════════════════
# COINGECKO HELPER — central rate-limit-aware request function
# ════════════════════════════════════════════════════════════════════

def _cg_get(path, params=None, timeout=15):
    """CoinGecko request with optional demo-key + exponential back-off.

    Set COINGECKO_API_KEY in st.secrets to use the demo tier (fewer 429s).
    Free public tier: up to 3 retries with 2 / 4 / 8 s back-off on 429.
    """
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 429:
                _time.sleep(2 ** (attempt + 1))   # 2 s, 4 s, 8 s
                continue
            return None
        except Exception:
            if attempt < 2:
...
def _yahoo_quote_uncached(ticker):
    """Non-cached variant — safe to call from ThreadPoolExecutor threads.

    FIX: @st.cache_data functions called inside threads emit 'missing
    ScriptRunContext' warnings because the cache decorator tries to access
    the Streamlit session state. This plain function avoids that entirely.
    """
    TICKER_MAP = {"DXY": "DX-Y.NYB", "$DXY": "DX-Y.NYB"}
    t = TICKER_MAP.get(ticker, ticker)
    try:
...
def get_heatmap_data():
    """Fetch S&P sector heatmap — concurrent batched fetching.

    FIX: Uses _yahoo_quote_uncached inside threads to eliminate the
    'missing ScriptRunContext' log flood caused by @st.cache_data in threads.
    """
    ...
    with ThreadPoolExecutor(max_workers=12) as pool:
        futures = {pool.submit(_yahoo_quote_uncached, tkr): (sector, tkr) for sector, tkr in all_jobs}
...
def score_options_chain(calls_df, puts_df, current_price, vix=None):
    ...
    def _score_side(df, side):
        ...
        if current_price and current_price > 0 and "strike" in df.columns:
            ...
            df["_delta_proxy"] = 0.5
        df["_score"] = (df["_norm_voi"] * w1 + df["_iv_pct"] * w2
                        - (df["_delta_proxy"] - 0.5).abs() * w3)
        rows = []
        for _, r in df.iterrows():
            rows.append({
                "strike": float(r.get("strike", 0)), "lastPrice": float(r.get("lastPrice", 0)),
                "bid": float(r.get("bid", 0)), "ask": float(r.get("ask", 0)),
                "volume": int(r.get("volume", 0)), "openInterest": int(r.get("openInterest", 0)),
                "iv": float(r.get("impliedVolatility", 0)), "voi": round(float(r.get("_voi", 0)), 2),
                "score": round(float(r.get("_score", 0)), 4), "side": side,
            })
        return rows
...
# ════════════════════════════════════════════════════════════════════
# CRYPTO — all free, no paid API keys required
#   • Fear & Greed  : alternative.me
#   • Markets/Global: CoinGecko free tier (via _cg_get with back-off)
#   • Exchanges     : CoinGecko free tier
#   • Whale trades  : Coinbase Exchange public REST
#   • Funding rates : Bybit v5 (free)
#   • Open interest : Bybit v5 (free)
#   • Liquidations  : Bybit v5 recent-trade (free) — replaces Coinglass
# ════════════════════════════════════════════════════════════════════
...
@st.cache_data(ttl=600)
def crypto_markets():
    """Top 20 coins by market cap — CoinGecko free API with rate-limit back-off."""
    data = _cg_get(
        "/coins/markets",
...
@st.cache_data(ttl=600)
def crypto_global():
    """Global crypto market stats — CoinGecko free API."""
    data = _cg_get("/global")
    return data.get("data", {}) if isinstance(data, dict) else {}

@st.cache_data(ttl=600)
def get_exchange_netflow():
    """Top 10 exchanges by BTC volume — CoinGecko free API."""
    data = _cg_get("/exchanges", params={"per_page": 10, "page": 1})
    if not isinstance(data, list):
        return []
    result = [{"name": ex.get("name", ""),
               "btc_vol_24h": float(ex.get("trade_volume_24h_btc", 0) or 0),
               "trust_score": int(ex.get("trust_score", 0) or 0)} for ex in data]
...
def get_whale_trades(symbol="BTCUSDT", min_usd=500_000):
    ...
    Returns top 25 by USD value. No API key required.
    """
    ...
    try:
        ...
        for t in r.json():
            ...
            iso  = t.get("time", "")
            try:
                time_str = datetime.fromisoformat(iso.replace("Z", "+00:00")).strftime("%H:%M:%S")
            except:
                time_str = str(iso)[:8]
            result.append({"time": time_str, "side": side,
                           "qty": round(qty, 4), "usd": round(usd, 2), "price": round(price, 2)})
...
def get_funding_rates():
    """Bybit v5 perpetual funding rates — free, no API key required."""
    try:
        ...
        for sym in _FUNDING_SYMBOLS:
            ...
            result.append({"symbol":   sym.replace("USDT", ""),
                           "rate_pct": rate_pct,
...
def get_open_interest():
    """Bybit v5 open interest — free, no API key required."""
    try:
        tr = requests.get("https://api.bybit.com/v5/market/tickers",
                          params={"category": "linear"}, timeout=10)
        tr.raise_for_status()
        marks = {t["symbol"]: float(t.get("markPrice", "0") or "0")
                 for t in tr.json().get("result", {}).get("list", [])}
        result = []
...
def get_liquidations():
    """Liquidation proxy via Bybit v5 recent-trade — free, no API key required.

    Coinglass was removed because their public endpoint now requires a paid key.
    This implementation fetches the last 500 trades per coin from Bybit and
    aggregates approximate long/short liquidation USD volume.

    Convention (Bybit linear):
    ...
    for coin in _LIQ_COINS:
        ...
        try:
            r = requests.get(
                ...
                params={"category": "linear", "symbol": sym, "limit": 500},
                timeout=10)
            r.raise_for_status()
            ...
            for t in trades:
                try:
                    ...
                    continue
            result[coin] = {"long_liq":  round(long_liq,  2),
...
def _alpaca_headers():
    try:
        return {"APCA-API-KEY-ID":     st.secrets["ALPACA_API_KEY"],
...
def get_stock_snapshot(symbol="SPY"):
    ...
    try:
        r = requests.get(f"https://data.alpaca.markets/v2/stocks/{symbol}/snapshot",
                         headers=headers, timeout=10)
        if r.status_code != 200: return None
        data = r.json()
        lt, db, mb = data.get("latestTrade", {}), data.get("dailyBar", {}), data.get("minuteBar", {})
        return {"price": _safe_float(lt.get("p")), "vwap":  _safe_float(db.get("vw")),
                "open":  _safe_float(db.get("o")),  "high":  _safe_float(db.get("h")),
                "low":   _safe_float(db.get("l")),  "close": _safe_float(db.get("c")),
                "volume": int(_safe_float(db.get("v"))), "minute_vwap": _safe_float(mb.get("vw"))}
    except Exception:
        return None
...
def get_spx_metrics():
    ...
    return {"spot":   round(snap["price"] * 10, 2), "vwap":   round(snap["vwap"] * 10, 2),
            "open":   round(snap["open"]  * 10, 2), "high":   round(snap["high"] * 10, 2),
            "low":    round(snap["low"]   * 10, 2), "volume": snap["volume"]}
...
def fetch_0dte_chain(underlying="SPY"):
    ...
    try:
        rc = requests.get("https://paper-api.alpaca.markets/v2/options/contracts",
                          headers=headers,
                          params={"underlying_symbols": underlying, "status": "active", "limit": 1000},
                          timeout=10)
        if rc.status_code != 200: return [], f"Alpaca API error (contracts): {rc.status_code}"
        dates = [c["expiration_date"] for c in rc.json().get("option_contracts", [])
                 if c.get("expiration_date", "") >= today_str]
        if not dates: return [], "No active option contracts found"
        target_expiry = sorted(set(dates))[0]
    ...
    try:
        ...
        if r.status_code != 200: return [], f"Alpaca API error (snapshots): {r.status_code}"
        data      = r.json()
        ...
        for sym, sd in snapshots.items():
            ...
            chain.append({"symbol": sym, "strike": strike, "type": opt_type,
                          ...
                          "oi": int(_safe_float(sd.get("openInterest", 0))),
...
def compute_max_pain(chain):
    ...
    for opt in chain:
        k = opt["strike"]
        (call_oi if opt["type"] == "call" else put_oi)[k] = \
            (call_oi if opt["type"] == "call" else put_oi).get(k, 0) + opt.get("oi", 0)

    s0   = strikes[0]
    pain = (sum(call_oi.get(k, 0) * (s0 - k) for k in strikes if s0 > k) +
            sum(put_oi.get(k,  0) * (k - s0) for k in strikes if s0 < k))
    ...
    for i in range(1, len(strikes)):
        ...
        if pain < min_pain:
            min_pain, mp_strike = pain, strikes[i]
...
def fetch_vix_data():
    ...
    for attr, ticker in [("vix", "^VIX"), ("vix9d", "^VIX9D")]:
        try:
            h = yf.Ticker(ticker).history(period="5d")
            if not h.empty:
                result[attr] = round(h["Close"].iloc[-1], 2)
        except Exception:
            pass
...
def _score_option(opt, chain_ivs, spot_spy):
    ...
    f1       = _math.exp(-((abs_delta - 0.30) ** 2) / (2 * 0.10 ** 2))
    ...
    iv_ratio  = (iv / median_iv) if median_iv > 0 else 1.0
    f5        = max(0, min(1, 2 - iv_ratio))
...
def find_target_strike(chain, bias):
    ...
    scored = []
    for o in cands:
        s, bd = _score_option(o, chain_ivs, o["strike"])
        o2 = dict(o); o2["_score"] = s; o2["_breakdown"] = bd
        scored.append(o2)
...
def generate_recommendation(chain, spx_metrics, vix_data):
    ...
    gamma_flip    = (find_gamma_flip(gex_profile) or 0) * 10 or None
    max_pain      = (compute_max_pain(chain) or 0) * 10 or None
    ...
    if score >= 2:
        ...
        return {"recommendation": "NO TRADE — Mixed Signals",
                "rationale": "Confluence score is too low (-1 to +1). Market is mixed or choppy.",
    ...
    parts_map = {
        "bull": {"Spot > VWAP":          "SPX is above its daily VWAP",
        ...
        "bear": {"Spot < VWAP":          "SPX is below its daily VWAP",
    ...
    rationale = (f"Confluence score is **{score:+d}**. The market shows strong {direction} momentum "
                 f"because " + " and ".join(rp) + ".")

    target_pts = max(abs(int(hedge_wall - strike_spx)) if hedge_wall else 10, 1)
    stop_pts   = max(int(abs(strike_spx - max_pain))   if max_pain  else 10, 1)
    target_desc = f"Target {int(hedge_wall)} Hedging Wall (+{target_pts}pts)" if hedge_wall else "Target +10pts"
    stop_desc   = f"Stop-loss at Max Pain {int(max_pain)} (-{stop_pts}pts)"   if max_pain  else "Stop-loss at -10pts"

    greeks_line  = (f"Δ={target['delta']:+.3f}  Γ={target['gamma']:.4f}  "
                    f"Θ={target['theta']:.4f}  V={target['vega']:.4f}  IV={target['iv']:.1%}")
    scoring_line = (f"Score: {bd.get('total',0):.3f}  "
                    f"(Δ-fit:{bd.get('delta_score',0):.2f} Γ/Θ:{bd.get('gt_score',0):.2f} "
                    f"Liq:{bd.get('liq_score',0):.2f} Flow:{bd.get('flow_score',0):.2f} "
                    f"IV-val:{bd.get('iv_score',0):.2f})")
    stats_text   = (f"Greeks: {greeks_line}\n"
                    f"~{delta_pct}% P(ITM) | Γ/Θ: {bd.get('gt_ratio',0):.1f}x | "
                    f"Spread: {bd.get('spread_pct',0):.1f}% | Flow: {bd.get('flow_ratio',0):.2f}x OI\n"
                    f"{scoring_line}")

    return {"recommendation": f"RECOMMENDATION: BUY {int(strike_spx)} {opt_label}",
